...
void DrawSkyBackgroundLayer(World *world, RETRO_Camera *camera, int textureIndex, unsigned int textureObj, float scroll)
{
	...
	for (int stack = 0; stack < stacks; stack++) {
		...
		for (int slice = 0; slice <= slices; slice++) {
			...
			glTexCoord2f(s, t);
			glVertex3f(camera->head[0] + dir1[0] * radius,
					camera->head[1] + dir1[1] * radius,
					camera->head[2] + dir1[2] * radius);

			SkyTexCoord(world, textureIndex, dir0, scroll, &s, &t);
			glTexCoord2f(s, t);
			glVertex3f(camera->head[0] + dir0[0] * radius,
					camera->head[1] + dir0[1] * radius,
					camera->head[2] + dir0[2] * radius);
		}
		glEnd();
	}
}
